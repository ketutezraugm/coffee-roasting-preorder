...
Three services, matching the bounded contexts found in event storming (see `docs/coupling.md` for how
this changed while building): `ordering` folds payment into its `Order` aggregate rather than making
Payment a fourth service.
...
| `production` | Roasting and packing | 3002 | `production_db` / `production_user` |
| `fulfilment` | Delivering: recipient details, tracking number, delivery state | 3003 | `fulfilment_db` / `fulfilment_user` |

Each service has its own database and credentials and never touches another's data. They talk only over
HTTP, following the OpenAPI files in [contracts/](contracts/). The client is a set of Node scripts in [scripts/](scripts/).

## Ownership

| Service | Owner | Reviewer |
|---|---|---|
| `ordering` | [name] | [name] |
| `production` | [name] | [name] |
| `fulfilment` | [name] | [name] |

Three services, three members: each owns one service and reviews another's. Contracts, docs, infra and
the client scripts are shared and committed by whoever changes them. Each owner runs Claude Code on their
own service and commits under their own git identity.
...
- Ports `3001`, `3002`, `3003` and `5432` free.
...
Option A, Docker (creates all three databases and roles automatically on first start):
...
Either way this creates `ordering_db`, `production_db` and `fulfilment_db`, each owned by its own role
(dev passwords are in `infra/init.sql`), and each role can connect to its own database only.

### 2. Install dependencies and create env files

Run this once in each service folder and in `scripts`:

```
cd services/fulfilment && npm install && cp .env.example .env && cd ../..
...
Use three terminals. **Start order: `fulfilment`, then `production`, then `ordering`.** Each service starts
fine with the others down; a downstream call just answers 502 until the callee is up.

Terminal 1:

```
cd services/fulfilment
npm start
```
...
Terminal 3. For the demo, shorten the payment hold to 20 seconds (the real default is 12 hours, see below):
...
Each service applies its own database migrations at startup and exposes `GET /health`. Per-service details
and environment variables: [services/ordering/README.md](services/ordering/README.md),
[services/production/README.md](services/production/README.md), [services/fulfilment/README.md](services/fulfilment/README.md).
...
npm run check-isolation    # each service's DB credentials are refused by the other services' databases
```

## Demonstrate independent deployability (B4)

Change one service, restart only it, and the others keep running: see [docs/b4-demo.md](docs/b4-demo.md).
...
| `services/fulfilment/` | Fulfilment service: same |
...
- **`EADDRINUSE`:** another process uses port 3001, 3002 or 3003. Stop it, or change `PORT` in the service's
  `.env` (and the matching `*_URL` in whichever other services and scripts call it: `ORDERING_URL`,
  `PRODUCTION_URL`, `FULFILMENT_URL`).
